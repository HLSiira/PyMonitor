...
def print_result(pkgs):
    ...
    if not pkgs:
        ...
        text.append('{} updatable package(s) / {} security update(s).'.format(
            len(pkgs), len(security_updates)))

        # text.append()

        # text.append('<hr noshade color="#FFFFFF" width="100%" size="1" style="padding:0; margin:8px 0 8px 0; border:none; width:100%; height: 1px; color:#FFFFFF; background-color: #FFFFFF" /><br />')
        text.append('-' * 80)
        ...
        text.append('-' * 80)
        for pkg in pkgs:
            text.append('{:<30}{:<20}{:<20}{:<10}'.format(pkg.get('name'),
                                                          pkg.get(
                                                              'current_version'),
                                                          pkg.get(
                                                              'candidate_version'),
