...
def calculateAge(anniversary, today):
    age = (
        today.year
        - anniversary.year
        - ((today.month, today.day) < (anniversary.month, anniversary.day))
    )
    return age
...
def checkDate():
    ...
    with open(storagePath, newline="") as csvfile:
        ...
        for row in reader:
            ...
            if (
                0 <= (anniversary - today).days < threshold
    ...
    if any(events) or args.test:
        ...
        for date in events.keys():
            ...
            if events[date][0]["isToday"]:
                message += f"{newline}<b><font color='#0074D9'>- {date.strftime('%b %d')} (today):</font></b>"
            else:
                message += f"{newline}<b>- {date.strftime('%b %d')}:</b>"
